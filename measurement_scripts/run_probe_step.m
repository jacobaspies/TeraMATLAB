...
tic
clearvars -except probe device sr830 % Clear all variables except hardware objects
close all
start_time = datestr(now); % Measurement start time

%% Experiment Parameters
N_avg = 1; % Number of iterations requested
t_probe = 748:0.05:758;
...
%% Save data to file
if save
    save_data([t_probe data_X_avg],'.timx')
    save_data([t_probe data_Y_avg],'.timy')
    save_data([t_probe data_X],'.timsx')
    save_data([t_probe data_Y],'.timsy')
    
    write_THz_metadata(sr830,memo,N_avg,tc_to_wait,N_samples,iterations_completed,start_time);
end
