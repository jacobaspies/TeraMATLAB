...
N_avg = 5; % Number of iterations requested
positions = 25:0.05:30;
...
save = 1; % Boolean to determine whether data is saved (to be changed to a text query later)
memo = "Line scan knife edge, z=9.5 mm";
...
%% Save data to file
if save
    save_data([positions data_X_avg],'.timx')
    save_data([positions data_Y_avg],'.timy')
    save_data([positions data_X],'.timsx')
    save_data([positions data_Y],'.timsy')
    write_THz_metadata(sr830,memo,N_avg,tc_to_wait,N_samples,iterations_completed,start_time,zaber_position);
end
